...
volatile static uint8_t enc_flag = 0;
...
int main(void)
{
  ...
  enc_handle.Init.InterruptEnableBits = EIE_PKTIE;

  //rev = enc_read_reg(EREVID);
  HAL_Delay(100);
  ...
  while (1)
  {
	  if(enc_flag)
	  {
		  ...
		  //if (enc_handle.interruptFlags & EIR_PKTIF)
		  //{
